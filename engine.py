...
def play_game(player, entities, game_map, message_log, game_state, root_console, con, panel, constants):
    ...
    while not tdl.event.is_window_closed():
        if fov_recompute:
            game_map.compute_fov(player.x, player.y, fov=constants['fov_algo'], radius=constants['fov_radius'], light_walls=constants['fov_light_walls'])

        render_all(con, panel, entities, player, game_map, fov_recompute, root_console, message_log, constants['screen_width'],
        ...
        if take_stairs and game_state == GameStates.PLAYERS_TURN:
            for entity in entities:
                if entity.stairs and entity.x == player.x and entity.y == player.y:
                    game_map, entities = next_floor(player, message_log, entities.stairs.floor, constants)
                    fov_recompute = True
                    con.clear()
